# Generated by Powerlevel10k configuration wizard on 2025-07-21 at 09:51 PDT.
...
# round separators, slanted heads, flat tails, 2 lines, solid, no frame, dark-ornaments,
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    kubie_context           # kubernetes context (only when in kubie shell)
  ...
  # Show kubecontext only when inside a kubie shell
  typeset -g POWERLEVEL9K_KUBECONTEXT_SHOW_ON_COMMAND='kubectl|helm|kubens|kubectx|k9s|stern|kubecolor|kubie'
  ...
  # Always show namespace (including "default")
  ...
  ##################################[ kubie_context: kubie k8s context ]##################################
  # Custom segment: only shows kubernetes context when inside a kubie shell (KUBIE_ACTIVE is set)
  function prompt_kubie_context() {
    [[ -n "$KUBIE_ACTIVE" ]] || return
    local ctx ns
    ctx="$(kubie info ctx 2>/dev/null)" || return
    ns="$(kubie info ns 2>/dev/null)"
    p10k segment -b 5 -f 7 -i '☸' -t "${ctx}/${ns:-default}"
  }

  function instant_prompt_kubie_context() {
    # Skip in instant prompt (commands are slow)
  }

  typeset -g POWERLEVEL9K_KUBIE_CONTEXT_FOREGROUND=7
  typeset -g POWERLEVEL9K_KUBIE_CONTEXT_BACKGROUND=5
